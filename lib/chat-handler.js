...
const SYSTEM_PROMPT = `You are the on-page assistant embedded in a school science-fair project website: the "Laser-LDR Turbidity Tester," built by students Anay, Aarnav, Govind, and Manan at Bodhi International School (CBSE, grades 9-12).
...
export async function handleChat(messages, apiKey, model) {
  if (!apiKey) {
    throw new Error('Server is not configured with an xAI API key.');
  }
  ...
  if (!response.ok) {
    ...
    const err = new Error(`xAI API error (${response.status}): ${text.slice(0, 300)}`);
  ...
  if (!reply) throw new Error('xAI returned an empty response.');
  return reply;
}
